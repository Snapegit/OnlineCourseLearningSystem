			...
			<el-row>
				...
				<el-col :span="12">
					<el-form-item label="作业附件" prop="zuoyefujian">
						<uploads
							:disabled="!isAdd||disabledForm.zuoyefujian?true:false"
							type="file"
							action="file/upload" 
							tip="请上传作业附件" 
							:limit="1" 
							style="width: 100%;text-align: left;"
							:fileUrls="form.zuoyefujian?form.zuoyefujian:''" 
							@change="zuoyefujianUploadSuccess">
						</uploads>
					</el-form-item>
				</el-col>
				<el-col :span="24">
				...
				<el-col :span="24">
					<el-form-item label="作业内容" prop="zuoyeneirong">
						<editor class="list_editor" :value="form.zuoyeneirong" placeholder="请输入作业内容" :readonly="!isAdd||disabledForm.zuoyeneirong?true:false"
							@change="(e)=>editorChange(e,'zuoyeneirong')"></editor>
					</el-form-item>
				</el-col>
			</el-row>
...
<script setup>
	...
	//作业附件上传回调
	const zuoyefujianUploadSuccess=(e)=>{
		form.value.zuoyefujian = e
	}
	//methods
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.zuoyeneirong = res.data.data.zuoyeneirong.replace(reg,'../../../cl11500170/file');
	...
	const save=()=>{
		...
		if(form.value.zuoyefujian!=null) {
			form.value.zuoyefujian = form.value.zuoyefujian.replace(new RegExp(context?.$config.url,"g"),"");
		}
